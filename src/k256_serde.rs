use ecdsa::elliptic_curve::{generic_array::GenericArray, group::GroupEncoding};
use k256::elliptic_curve::sec1::{FromEncodedPoint, ToEncodedPoint};
use rand::{CryptoRng, RngCore};
use serde::{de, de::Error, de::Visitor, Deserialize, Deserializer, Serialize, Serializer};
use typenum::{bit::{B0, B1}, UInt, UTerm};
use zeroize::Zeroize;
...
impl SecretScalar {
    pub fn random_with_thread_rng() -> Self {
        Self(Scalar(
            <k256::Scalar as k256::elliptic_curve::Field>::random(rand::thread_rng()),
        ))
    }

    pub fn random(rng: impl CryptoRng + RngCore) -> Self {
        Self(Scalar(
            <k256::Scalar as k256::elliptic_curve::Field>::random(rng),
        ))
    }
}
